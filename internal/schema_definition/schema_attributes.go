...
const (
	...
	AttributeLoginPasswordWOVersion        = "password_wo_version"
	...
	DescriptionLoginPasswordWO               = "Login password (write-only). Not stored in state. Requires `password_wo_version` to be set, and bumped whenever the value changes, so Terraform can detect an update. Conflicts with `password`."
	DescriptionLoginPasswordWOVersion        = "Used together with `password_wo` to trigger an update: change this value whenever `password_wo` changes."
