...
const (
	...
	AttributeAttachmentDynamicFile         = "dynamic_file"
	...
	DescriptionItemAttachmentDynamicFile     = "File Path to the attachment, for use when it may not be known or created until apply."
	DescriptionItemAttachmentDataHash        = "Calculated SHA1 hash of the attachment uploaded from `dynamic_file`. This can be specified to ensure update of attachment if `dynamic_file` changes on disk. Also, a special value `ignore` can be specified, and the attachment will not be updated when `content_hash` changes are detected."
